...
import { classifyContent, getAllowedCategories, getAiStatus } from './aiGuard.js';
...
async function checkGateStatus() {
  ...
  let valid;
  if (running.size === 0) {
    valid = true; // No browsers running
  } else {
  ...
  // Auto-disable focus mode if unguarded browser detected
