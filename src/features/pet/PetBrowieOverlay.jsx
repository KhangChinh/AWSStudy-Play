...
const PetBrowieOverlay = ({ equippedPet }) => {
  ...
  const pet = React.useMemo(() => {
    ...
    const allAnims = {
      ...
      Idle: { file: dbPet.assets?.idle, frames: 2, speed: 300 },
      Walk: { file: dbPet.assets?.walk, frames: 4, speed: 150 },
      Run: { file: dbPet.assets?.run, frames: 4, speed: 100 },
      Hurt: { file: dbPet.assets?.recievedamage, frames: 4, speed: 200 },
      Death: { file: dbPet.assets?.die, frames: 6, speed: 150, loop: false },
      Jump: { file: dbPet.assets?.jump, frames: 11, speed: 100 },
      Attack: { file: dbPet.assets?.attack, frames: 6, speed: 100 },
      Happy: { file: dbPet.assets?.happy, frames: 2, speed: 200 }
    };
  ...
  useEffect(() => {
    ...
    const pickAction = () => {
      ...
      } else if (rand < 0.4 && pet.animations.Happy) {
         nextState = 'Happy';
         targetRef.current = null;
         const skillFrames = actualFramesRef.current['CarrotSkill'] || pet.animations.Happy.frames;
         delay = (skillFrames * pet.animations.Happy.speed) || 2000;
...
export default PetBrowieOverlay;