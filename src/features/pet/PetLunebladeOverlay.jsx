...
const PetLunebladeOverlay = ({ equippedPet }) => {
  ...
  const pet = React.useMemo(() => {
    ...
    const allAnims = {
      ...
      Idle: { file: dbPet.assets?.idle, frames: 7, speed: 150 },
      Walk: { file: dbPet.assets?.run, frames: 8, speed: 100 },
      Run: { file: dbPet.assets?.run, frames: 8, speed: 100 },
      Dash: { file: dbPet.assets?.dash, frames: 12, speed: 80 },
      Hurt: { file: dbPet.assets?.hurt, frames: 3, speed: 200 },
      Death: { file: dbPet.assets?.death, frames: 18, speed: 100, loop: false },
      Jump: { file: dbPet.assets?.jump, frames: 4, speed: 150 },
      Attack: { file: dbPet.assets?.['attack 1'], frames: 10, speed: 100 },
      Attack2: { file: dbPet.assets?.['attack 2'], frames: 15, speed: 100 }
    };
  ...
  useEffect(() => {
    ...
    const pickAction = () => {
      ...
      } else if (rand < 0.4 && pet.animations.Dash) {
         nextState = 'Dash';
         targetRef.current = null;
         const skillFrames = actualFramesRef.current['CarrotSkill'] || pet.animations.Dash.frames;
         delay = (skillFrames * pet.animations.Dash.speed) || 2000;
  ...
  const style = {
    ...
    transform: `scale(${pet.scale || 1.5}) scaleX(${dir === 'left' ? -1 : 1}) translateY(${pet.translateY || 0}px)`,
  ...
  if (animConfig.type !== 'gif') {
    const activeFramesCount = actualFrames[state] || animConfig.frames;
    style.backgroundSize = `${activeFramesCount * 100}% 100%`;
...
export default PetLunebladeOverlay;