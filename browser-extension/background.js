    ...
    case 'STRIKE_REPORT':
      ...
      if (wsConnection && wsConnection.readyState === WebSocket.OPEN) {
        wsConnection.send(JSON.stringify({
          ...
          reason: message.reason || 'Stayed on blocked video for 10+ seconds'
        }));
        console.log('[FocusGuard] Strike report forwarded to Desktop App');
      } else {
        console.warn('[FocusGuard] Cannot forward strike: Desktop App not connected');
      }
      return true;
  }
});
